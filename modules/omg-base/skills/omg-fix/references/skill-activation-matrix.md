...
|----------|---------------|
| `omg-debugger` | Root cause unclear, need deep investigation (Step 2) |
...
| `omg-researcher` | External docs needed, latest best practices (Deep only) |
| `omg-planner` | Complex fix needs breakdown, multiple phases (Deep only) |
| `omg-tester` | After implementation, verify fix works (Step 5) |
| `ck:code-review` | After fix, verify quality and security (Step 5) |
| `omg-git-manager` | After approval, commit changes (Step 6) |
| `omg-docs-manager` | API/behavior changes need doc updates (Step 6) |
| `omg-project-manager` | Major fix impacts roadmap/plan status (Step 6) |
| `omg-fullstack-developer` | Parallel independent issues (each gets own agent) |
...
| 2+ independent issues | Task trees + `omg-fullstack-developer` agents per issue |
...
| Standard | Above + Tasks, `ck:problem-solving` (auto), `ck:project-management`, `omg-tester`, parallel `Explore` |
| Deep | All above + `ck:brainstorm`, `ck:context-engineering`, `omg-researcher`, `omg-planner` |
| Parallel | Per-issue Task trees + `ck:project-management` + `omg-fullstack-developer` agents + coordination via `TaskList` |
...
| Step 6: Finalize | Report → `omg-docs-manager` → `TaskUpdate` → `omg-git-manager` → run `omg-watzup` or record session summary |
...
| "latest docs", "best practice" | `omg-researcher` subagent |

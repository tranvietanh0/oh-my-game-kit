...
For fixing failing tests and test suite issues. Uses native Codex Tasks for phase tracking.
...
Use `omg-tester` agent. Fix all syntax errors before running tests.
...
Use `omg-debugger` agent for root cause analysis.
...
Use `omg-planner` agent for fix strategy.
...
Use `omg-tester` agent. If tests still fail → keep T5 `in_progress`, loop back to Step 2.

`TaskUpdate(T5, status="completed")`

### Step 6: Review
`TaskUpdate(T6, status="in_progress")`
Use `omg-code-reviewer` agent.

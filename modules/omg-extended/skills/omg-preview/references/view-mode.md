...
The skill is located at `$HOME/.agents/skills/markdown-novel-viewer/`.
...
```bash
node $HOME/.agents/skills/markdown-novel-viewer/scripts/server.cjs --stop
```
...
if [[ -d "$INPUT_PATH" ]]; then
  node $HOME/.agents/skills/markdown-novel-viewer/scripts/server.cjs \

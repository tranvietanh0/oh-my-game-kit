...
Not for: full project cloning (use `omg-repomix` for full codebase snapshots, or clone directly), simple file copy, or package installation.

## Usage

```text
omg-xia <github-url|owner/repo|local-path> [feature-description] [--compare|--copy|--improve|--port] [--auto|--fast]
```
...
1. Pack the source with `omg-repomix`.
...
3. Use the `omg-researcher` agent to understand purpose, trade-offs, and community context.
4. Use `omg-scout` on the local project to map architecture, similar features, and integration points.
...
If you delegate to `omg-researcher`, `scout`, or `omg-planner`, pass:
- work context
- reports path
- plans path
...
- activate `omg-think` to trace multi-step flows
...
If there are 3 or more competing concerns, use the `omg-brainstormer` agent or an inline trade-off exercise.
Do not invoke `omg-brainstorm` from inside `xia`; that skill can create its own planning handoff and break `xia`'s phase ownership.
...
- other modes: present the plan path and hand implementation to `omg-cook`

Implementation handoff text:

```text
Plan ready at ./plans/<plan-dir>/plan.md. To implement, run omg-cook <plan-path>.
```

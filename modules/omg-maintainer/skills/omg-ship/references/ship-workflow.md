...
2. Delegate to `omg-tester` subagent — don't inline test execution
...
2. Delegate to `omg-code-reviewer` subagent with the diff
3. Two-pass model:
...
1. Invoke `omg-docs update` via `omg-docs-manager` subagent in background:

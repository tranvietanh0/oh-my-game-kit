...
cycle = 0
LOOP:
  1. Run omg-code-reviewer → score, critical_count, warnings, suggestions
  ...
  3. AskUserQuestion (header: "Review & Approve"):
     IF critical_count > 0:
       - "Fix critical issues" → fix, re-run omg-tester, cycle++, LOOP
       - "Fix all issues" → fix all, re-run omg-tester, cycle++, LOOP
...
cycle = 0
LOOP:
  1. Run omg-code-reviewer → score, critical_count, warnings

...
Use `mcp__context7__` MCP for library docs and spawn `omg-researcher` agent via `Task` tool for broader research. Research:
...
| `.cjs / .js / .mjs` | Line 2 (after shebang if present): `// omg-origin: kit=<name> \| repo=<org/repo> \| module=<m> \| protected=<true\|false>` |
| `.sh / .py / .yml / .yaml` | Line 2 (after shebang): `# omg-origin: kit=<name> \| repo=<org/repo> \| module=<m> \| protected=<true\|false>` |
| `.json` | Top-level `_origin: { kit, repository, module, protected }` object |

**Why required:** the `validate-origin-injection-coverage` CI gate runs on PR branches and fails the build if a new file lacks the marker. CI's `inject-origin-metadata.cjs` overwrites the values post-merge — so hand-stamped values can be best-effort, but the marker MUST be present. AGENTS.md "do not hand-author" applies to existing files on `main`, not new files in PRs. Memory: `feedback_origin_marker_handauthor.md`.

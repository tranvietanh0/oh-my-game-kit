...
Full pipeline for moderate complexity issues. Uses native Codex Tasks for phase tracking.
...
2. Activate `ck:debug` skill. Use `omg-debugger` subagent if needed.
...
Use `omg-code-reviewer` subagent.
...
- Update docs if needed via `omg-docs-manager`
- Ask to commit via `omg-git-manager` subagent
- Record session summary (run `omg-watzup` at session wrap)
...
| 2 | `ck:debug`, `ck:sequential-thinking`, `omg-debugger` subagent, parallel `Explore`, (`ck:problem-solving` auto) |
| 3 | `ck:problem-solving` (if stuck), `ck:sequential-thinking` (complex logic) |
| 4 | `omg-tester` subagent, parallel `Bash` verification |
| 5 | `omg-code-reviewer` subagent |
| 6 | `ck:project-management`, `omg-git-manager`, `omg-docs-manager` subagents |

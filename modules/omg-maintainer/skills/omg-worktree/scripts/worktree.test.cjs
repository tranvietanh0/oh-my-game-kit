 ...
 * Run: node .agents/skillsomg-worktree/scripts/worktree.test.cjs
 */
...
const MONOREPO_DIR = '/home/kai/codexkit';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'codexkit-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from codexkit-engineer submodule
  const submodulePath = '/home/kai/codexkit/codexkit-engineer';

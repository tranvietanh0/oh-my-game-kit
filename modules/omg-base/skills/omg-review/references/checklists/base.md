...
### Hardcoded Mappings (CRITICAL for OMG hooks)
...
- Inline `.agents/` path construction (`path.join(cwd, '.agents')`) instead of using `resolveCodexDir()` from shared module
- Hardcoded string literals for file/directory names that exist as constants in `OMG` from `telemetry-utils.cjs`

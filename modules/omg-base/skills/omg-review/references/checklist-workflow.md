...
- Pre-landing review (from `omg-ship` pipeline)
...
## Integration with omg-review

When invoked as part of standard code review, the checklist augments (not replaces) the existing scout → review → fix → verify pipeline. Checklist findings are merged with omg-code-reviewer's own findings.

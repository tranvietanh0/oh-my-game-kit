...
Full pipeline with research, brainstorming, and planning for complex issues. Uses native Codex Tasks with dependency chains.
...
Use `omg-researcher` subagent for external knowledge.
...
Use `omg-planner` subagent to create implementation plan.
...
- Use `omg-docs-manager` subagent for documentation
- Use `omg-git-manager` subagent for commit
...
| 3 | `omg-researcher` (runs parallel with steps 1+2) |
| 4 | `ck:brainstorm` |
| 5 | `omg-planner` |
| 6 | `ck:problem-solving`, `ck:sequential-thinking`, `ck:context-engineering` |
| 7 | `omg-tester`, parallel `Bash` verification |
| 8 | `omg-code-reviewer` |
| 9 | `ck:project-management`, `omg-docs-manager`, `omg-git-manager` |
